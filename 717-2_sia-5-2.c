...
void printStraightTreeBypass(tree* t)
{
  stack *s1 = malloc(sizeof(stack));
  nodeStack* tmp;
  push_back_stack(s1, t->root);
  tmp = s1->head;
  stack *s2 = malloc(sizeof(stack)); // хранит корни необойдённых ветвей
  while(1)
  {
    if (tmp->ndTree->left)  //qw?
    {
      push_back_stack(s1, tmp->ndTree->left);
    }
    else if (tmp->ndTree->right) //qw
    {
      push_back_stack(s1, tmp->ndTree->right);
    } 
    while (tmp->ndTree->left)
    {
      if (tmp->ndTree->right) push_back_stack(s2, tmp->ndTree->right);
      tmp = tmp->next;
      if (tmp->ndTree->left) push_back_stack(s1, tmp->ndTree->left);
    }
    if (s2->tail)
    {
       push_back_stack(s1, removeLastStackNode(s2)); 
       tmp = tmp->next;
    }
    else break; 
  }
